...
async def extract_all_thumb_bl_info(
    ...
    for thumb_video in page_soup.select('div[class*="item thumb-bl thumb-bl-video"]'):
        ...
        try:
            ...
            metadata = Metadata(
                views=convert_views(
                    thumb_meta_div.select_one("i.fa.fa-eye")
                    .parent.select_one("span")
                    .get_text(strip=True)
                ),
                duration=convert_duration(
                    thumb_meta_div.select_one("i.fa.fa-clock-o")
                    .parent.select_one("span")
                    .get_text(strip=True)
                ),
                upload_date=thumb_meta_div.select_one("i.fa.fa-calendar-o")
                .parent.select_one("span")
                .get_text(strip=True),
                extras={"preview": a_thumb_video.attrs.get("data-preview-custom")},
            )
        ...
        try:
            links = [
                ExternalLink(
                    name=d.attrs.get("title"),
                    url=DOMAIN + (d.attrs.get("href") or "/"),
                    extras={
                        "verified": "icon-verified"
                        in d.select_one("svg").attrs.get("class", "N/A")
                    },
                )
                for d in thumb_video.select("div.content-items a")
                if d
            ]
        except:
            pass
        ...
        try:
            video = ThumbVideo(
                ...
                thumbnail=Thumbnail(
                    url="https:" + img_thumb_video.attrs.get("data-original")
                ),
                metadata=metadata,
                links=links,
            )
