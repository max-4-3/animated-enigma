...
async def extract_all_thumb_videos(
    webpage: BeautifulSoup | Tag,
    ...
    for li in webpage.select("li.videoblock"):
        ...
        try:
            metadata = Metadata(
                views=convert_views(
                    get_text_wrapper(
                        lambda: li.select_one("span.views var").get_text(strip=True), 0
                    )
                ),
                duration=convert_duration(
                    get_text_wrapper(
                        lambda: li.select_one("var.duration").get_text(strip=True), 0
                    )
                ),
                upload_date=get_text_wrapper(
                    lambda: li.select_one("var.added").get_text(strip=True),
                    "A Long Time Ago...",
                ),
                extras={
                    "rating": get_text_wrapper(
                        lambda: li.select_one("div.value").get_text(strip=True), 0
                    )
                },
            )
        ...
        try:
            video = ThumbVideo(
                ...
                title=get_text_wrapper(
                    lambda: li.select_one("span.title a").get_text(strip=True),
                    "No Title",
                ),
                url="https://"
                + DOMAIN
                + li.select_one("span.title a").attrs.get("href", "/"),
...
async def extract_videos_from_webpage(
    sem, session: aiohttp.ClientSession, page_link: str, **kwargs
):
    async with sem:
