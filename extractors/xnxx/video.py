...
async def get_resolutions(session, master_m3u8: str, **kw) -> Media:
    ...
    result = Media(base_url=master_m3u8, items=[])
    for i, line in enumerate(lines):
        if line.startswith("#EXT-X-STREAM-INF"):
            try:
                props = dict(
                    prop.split("=", 1) for prop in line.split(",") if "=" in prop
                )
                _, height = map(int, props["RESOLUTION"].split("x"))
                media_url = media_url = (
                    "/".join(master_m3u8.split("/")[:-1])
                    + ("/" if not lines[i + 1].startswith("/") else "")
                    + lines[i + 1]
                )
                result.items.append(
                    MediaItem(
    ...
    for var in vars:
        ...
        new_data.append(
            ThumbVideo(
                ...
                thumbnail=Thumbnail(
                    url=var.get("ip") or var.get("if") or var.get("il") or var.get("i")
                ),
    ...
    **kwargs,
    ...
    async with sem:
        ...
        try:
            ...
            is_hd = wrap(
                lambda: (
                    int(parts[1].replace("p", "") or 0) >= 1080
                    if len(parts) > 1
                    else False
                ),
                False,
            ...
            def get_int_value(selector):
                ...
                return (
                    int(el.text.strip().replace(",", "").replace(".", "")) if el else 0
                )
            ...
            hls_match = re.search(
                r"""setVideoHLS\(['"](.+?)['"]\);""",
                bg_div.prettify() if bg_div else "",
            )
            ...
            if recommendation and bg_div:
                try:
                    related_match = re.search(
                        r"""var\s+video_related\s*=\s*(\[.*?\]);""", bg_div.text
                    )
                    if related_match:
            ...
            return Video(
                ...
                metadata=metadata,
                thumbnail=Thumbnail(
                    url=soup.select_one('meta[property*="og:image"]').get("content")
                ),
                media=media,
                tags=tags,
                links=[],
