import aiohttp, asyncio, os, aiofiles, time, shutil, logging, re, random
...
async def download_segment(
    ...
    async with sem:
        try:
            ...
            if not name:
                segment_name = (
                    segment_url.split("/")[-1].split("?")[0]
                    or segment_url.split("/")[-1]
                )
            else:
                segment_name = name
    ...
    async with sem:
        ...
        if make_subfolder:
            download_dir = os.path.join(
                download_dir, "videos" if subfoler_name is None else subfoler_name
            )
            ...
            with tqdm(
                ...
                for name, link in m3u8_urls:
                    tasks.append(
                        asyncio.create_task(
                            download_segment(
                                download_sem, session, link, temp_dir, pbar, name=name
                            )
                        )
                    )
